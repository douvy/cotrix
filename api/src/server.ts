...
// Type declaration for chromium module
type ChromiumType = {
  args: string[];
  defaultViewport: {
    width: number;
...
// Import chromium with type assertion
...
async function initializeBrowser(): Promise<Browser> {
  try {
    ...
    if (isLocal) {
      // Local environment configuration - Mac specific
      launchOptions = {
        headless: true,
        args: [
          '--no-sandbox',
...
async function initializePage(browser: Browser): Promise<Page> {
  try {
    ...
    page.on('error', (err: Error) => console.error('Page error:', err));
...
async function extractCodeFromPopup(page: Page): Promise<string | null> {
  try {
...
async function handleVoucherCard(page: Page, card: ElementHandle<Element>, browser: Browser): Promise<CouponData | null> {
  try {
...
async function scrapeCoupons(storeUrl: string): Promise<CouponData[]> {
  ...
  try {
    ...
    try {
      ...
      const fallbackCodes = await page.$$eval('h3, span, div', (elements: Element[]) =>
        elements
          .map(el => el.textContent?.trim())
          .filter((text): text is string => text !== null && text !== undefined && /^[A-Z0-9-]{4,15}$/.test(text))
          .map(code => ({
            code,
